...
2022.5.3 又去掉了几个冗余的方法。话说为什么我感觉我明明在去掉多余的代码，可代码量却一直没变啊。。。
    已知bug: 在exe中或直接使用本文件时，Maker.batch无论设置的是什么，只要设置了就一定会变成True。不想修这个bug，主要是没意义。。"""
...
class Structure:
    ...
    def mk_struct(self, picture: np.ndarray, times=0, all_time=1) -> nbt.NBTFile:
        ...
        t = trange(self.struct_size[1], postfix={"Now file": times + 1, "Total": all_time})
...
class Maker:
    ...
    def __update_out_path(self):
        """用于更新输出文件路径"""
        if self.__out_path in [Path(sys.argv[0]).parent, None]:
    ...
    def set_using_blocks(self, using_blocks: Union[str, list, tuple]):
        ...
        for block in using_blocks:
            if block == '':
                ...
                return None
            elif block not in self.__allow_blocks:
                raise NameError(f"NameError:\n\t未定义的方块空间'{block}'。")
    ...
    def set_path(self, path: str, path_type: str = "in"):
